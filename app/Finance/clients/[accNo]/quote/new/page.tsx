...
export default function NewQuotePage() {
  ...
  // ── Quote header fields ──
  // Initial date defaults are captured once so the dirty check below can
  // compare against them.
  const [initialExpectedDate] = useState(() => isoDatePlusDays(7));
  const [initialExpiryDate] = useState(() => isoDatePlusDays(30));
  ...
  const [warehouseNr, setWarehouseNr] = useState(0);
  ...
  const [repCodeOverride, setRepCodeOverride] = useState<string | null>(null);
  ...
  // "Dirty" means the user changed ANY editable field from its default —
  // items, money fields, and every delivery/detail input.
  const isDirty =
    lines.length > 0 ||
    notes.trim() !== "" ||
    customAddress !== null ||
