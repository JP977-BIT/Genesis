...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(
    `${apiConnection.api_base_url}/api/customers/customer/update`,
    {
      method: "POST",
      headers: {
  ...
  // Always read the body — Revelation sometimes returns errors as 400/500 with
  // a JSON body containing a useful message. Surface that instead of a generic 502.
  ...
  if (!upstream.ok) {
    return NextResponse.json(
      {
        success: false,
        message: friendlyRevelationMessage(result.message, "update"),
      },
      { status: 502 },
    );
  }
  ...
  if (!result.success) {
    return NextResponse.json(
      { success: false, message: friendlyRevelationMessage(result.message, "update") },
      { status: 400 },
    );
  }
