...
export async function GET(request: NextRequest) {
  ...
  const upstreamRes = await fetch(
    `${apiConnection.api_base_url}/api/customertransactions/invoices`,
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ companyNr, accNo, startingRow: 0, numberOrRecords: 0, warehouseNr: 0 }),
  ...
  // Filter by accNo in case the upstream ignores it
  if (data?.data?.invoices) {
    data.data.invoices = data.data.invoices.filter(
      (inv: { accNo: string }) => inv.accNo === accNo,
    );
  }
