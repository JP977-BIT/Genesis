...
export async function GET(request: NextRequest) {
  ...
  const stockItemCode = searchParams.get("stockItemCode");

  if (!companyNr || !stockItemCode) {
    return NextResponse.json(
      { message: "companyNr and stockItemCode are required" },
      { status: 400 },
    );
  }
  ...
  const stockItemResponse = await fetch(
    `${apiConnection.api_base_url}/api/stock/stockitem`,
    {
      method: "POST",
      headers: {
  ...
  const stockItemData = await stockItemResponse.json();
  return NextResponse.json({ success: true, data: stockItemData });
}
